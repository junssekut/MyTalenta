        ...
        <div class="px-4 py-8 relative z-10">
            <div class="max-w-7xl mx-auto">
                <div class="text-center">
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Shuttle Bus BCA</h1>
                    <p class="text-blue-700 text-lg font-medium">Pemesanan Transportasi Shuttle</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if ($bookingClosed)
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                ...
                <p class="text-red-700">Batas waktu pemesanan shuttle telah berakhir. Silakan hubungi PIC Shuttle untuk
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" wire:model.live="shuttleType" value="pulang" class="sr-only">
                            <div
                            ...
                            {{ $shuttleType == 'pulang' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <div class="text-center">
                                    <svg class="w-12 h-12 mx-auto mb-3 {{ $shuttleType == 'pulang' ? 'text-blue-600' : 'text-gray-400' }}"
                        ...
                        <!-- Shuttle Kembali -->
                        <label class="relative cursor-pointer">
                            <input type="radio" wire:model.live="shuttleType" value="kembali" class="sr-only">
                            <div
                                class="border rounded-lg p-6 transition-all
                        ...
                        @if (count($availableRoutes) > 0)
                            <div class="space-y-3">
                                @foreach ($availableRoutes as $route)
                                    <label class="relative cursor-pointer">
                                        <input type="radio" wire:model="selectedRoute" value="{{ $route->id }}"
                                            class="sr-only">
                                        <div
                                            class="border rounded-lg p-4 transition-all flex items-center justify-between
                            {{ $selectedRoute == $route->id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    <h3 class="font-semibold text-gray-900">{{ $route->destination }}
                                                    </h3>
                                                    ...
                                                    <p class="text-xs text-gray-500">
                                                        Jadwal:
                                                        {{ $route->departure_time ? \Carbon\Carbon::parse($route->departure_time)->format('H:i') : 'Belum ditentukan' }}
                                                    </p>
                                                </div>
                                            </div>
                                            @if ($route->capacity)
                                                <div class="text-right">
                                                    <span class="text-sm text-gray-600">Kapasitas:
                                                        {{ $route->capacity }}</span>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                <div class="text-sm text-gray-600">
                                    @if ($selectedRoute && $bookingDate)
                                        ...
                                        <p><strong>Tanggal:</strong>
                                            {{ \Carbon\Carbon::parse($bookingDate)->format('d M Y') }}</p>
                                        <p><strong>Jam:</strong>
